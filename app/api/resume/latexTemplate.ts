...
function splitAndCapitalize(description: string): string[] {
  ...
  // Split on periods
  let sentences = replaced
    .split(".")
    .map((s) => s.trim())
    .filter(Boolean);
  // Restore periods in tech stack names
  sentences = sentences.map((s) =>
    techStackExceptions.reduce(
      (acc, term) =>
        acc.replace(new RegExp(term.replace(/\./g, "<<DOT>>"), "g"), term),
      s
    )
  );
  // Capitalize
...
\\newenvironment{twocolentry}[2][]{\\onecolentry\\def\\secondColumn{#2}\\setcolumnwidth{\\fill, 4 cm}\\begin{paracol}{2}}{\\switchcolumn \\raggedleft \\secondColumn\\end{paracol}\\endonecolentry}
...
\\begin{onecolentry}
  \\begin{highlightsforbulletentries}
    ${data.about
      .flatMap((item) => splitAndCapitalize(item))
      .map((sentence) => `\\item ${escapeLatex(sentence.endsWith('.') ? sentence : sentence + '.')}`)
      .join("\n    ")}
...
${data.projects
  .map(
    (proj) =>
      `\\begin{twocolentry}{${new Date(proj.createdAt).getFullYear()}}\\textbf{${escapeLatex(proj.title)}}\\end{twocolentry}\n\\vspace{0.10 cm}\n\\begin{onecolentry}\n  \\begin{highlights}\n    ${splitAndCapitalize(
        proj.description
      )
